...
DEFAULT_BASE_DIR = Path("/Users/xingzhengpeng/CODEZONE/PCO/Power-Optimization/Recordings/huaweipura")
...
def _iter_recording_dirs(base_dir: Path) -> Iterable[RecordingDir]:
...
def _build_route_audits(base_dir: Path, route_root: Path) -> List[RouteAudit]:
    items_by_route: Dict[int, List[RecordingDir]] = defaultdict(list)
    for item in _iter_recording_dirs(base_dir):
        items_by_route[item.route_suffix].append(item)

    audits: List[RouteAudit] = []
    for route_suffix in sorted(items_by_route):
        items = sorted(
            items_by_route[route_suffix],
            key=lambda item: (
        ...
        if expected_segments is None:
            raise FileNotFoundError(f"Route file not found for suffix {route_suffix}: {route_root / f'{route_suffix}.py'}")
        ...
        for item, target_segment in matched_pairs:
            target_name = target_segment.segment_dir_name
            renames.append(
                PlannedRename(
                    source=item,
                    target_path=item.path.parent / target_name,
...
def _print_summary(audits: Sequence[RouteAudit]) -> None:
    for audit in audits:
        actual_counts = sorted({plan.source.file_count for plan in audit.renames} | {item.file_count for item in audit.extras})
        status = "ok"
        if audit.extras:
            status = "extra"
        ...
        print(
            f"[ROUTE {audit.route_suffix:02d}] {status} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rename huaweipura recording directories into natlan_rXX_<label><nn> format."
    )
    parser.add_argument(
        "--base-dir",
        type=Path,
        default=DEFAULT_BASE_DIR,
        help=f"Recording root to rename. Default: {DEFAULT_BASE_DIR}",
    )
    parser.add_argument(
        "--route-root",
        type=Path,
...
def main() -> int:
    ...
    mismatched_routes = [audit for audit in audits if audit.extras or audit.missing_labels]

    if not plans and not mismatched_routes:
